    ...
    const interval = setInterval(() => {
      if (type === "dishName" && this.state.typingIndexDish < text.length) {
        this.setState((prevState) => ({
          displayedDishName: prevState.displayedDishName +
            text[prevState.typingIndexDish],
          typingIndexDish: prevState.typingIndexDish + 1,
        }));
      } else if (
        type === "description" && this.state.typingIndexDesc < text.length
      ) {
        this.setState((prevState) => ({
          displayedDescription: prevState.displayedDescription +
            text[prevState.typingIndexDesc],
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{ position: "fixed", width: "20vw" }}
      >
        {/* Animated Dish Name */}
        <h1
          className={`content-title ${fadeInOutClass}`}
          style={{ fontSize: "80px", lineHeight: "80px", marginBottom: "5px" }}
        >
          {displayedDishName}
        </h1>
